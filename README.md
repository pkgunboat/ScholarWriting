...
ScholarWriting 的中文用户入口名是“学术写作助手”。它是一个面向学术写作的多 agent 工作流框架。推荐先把它安装到本机 Codex，然后在你自己的写作项目里用自然语言启动，让 agent 按“规划、写作、审阅、修订、再审阅”的流程推进学术文档。

`scholar-writing` 是机器 ID、CLI 命令和安装目录名。普通用户可以直接说“学术写作助手”。
...
框架会自动加载写作规则资料。`scholar_writing/resources/references/` 中的风格指南、国自然结构规范、去 AI 痕迹规则和章节句式模板会进入任务包，Codex 或 Claude Code agent 按这些规则写作、审阅和修订。用户只需要提供自己的材料、提纲或初稿。
...
开发者可以直接在本仓库里调试平台通用 repo-local skill 和 Codex agents：
...
任务包中的 `reference_inputs` 会列出本轮 agent 应读取的质量规则文件。新增规则资料时，开发者应维护 `scholar_writing/resources/config/reference_registry.yaml`，并确保路径仍为相对路径。
...
Codex 适配先建立共享的项目文件契约、状态机、CLI、schema 和 taskpack，再用 Codex 的 custom agent 机制执行任务。
...
这也是为什么不同框架的 agent 数量、调度方式和并行方式不同，但可以处理同一类写作项目。

...
`scholar_writing/resources/config/reference_registry.yaml` 是写作规则资料的索引。它把 `scholar_writing/resources/references/` 下的风格指南、国自然结构、去 AI 规则和句式模板映射到 action、agent role、项目类型和审阅维度。

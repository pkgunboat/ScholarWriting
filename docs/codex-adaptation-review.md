...
- `scholar-writing/skills/pipeline/SKILL.md` 使用 `allowed-tools: [Read, Write, Edit, Bash, Agent]`，这是 Claude Code 工具模型，不是 Codex 的 custom agent 配置。
...
- `skills/scholar-writing/references/`
- `skills/scholar-writing/scripts/` 或明确引用仓库内脚本
...
- `scholar-writing/scripts/run_pipeline.py`
- `scholar-writing/scripts/pipeline_state.py`
- `scholar-writing/scripts/score_reports.py`
...
仓库规则要求 Python 项目使用 `uv`。当前只有 `scholar-writing/scripts/requirements.txt`，且缺少 `jsonschema`。
...
- 安装：`uv sync`
- 测试：`uv run pytest -q`
- 校验：`uv run python scholar-writing/scripts/validate.py all examples/demo-nsfc-proposal`
...
现有 `scholar-writing/skills/pipeline/SKILL.md` 可以保留，但应明确：
...
- `uv run pytest -q` 通过。
- `uv run python scholar-writing/scripts/validate.py all examples/demo-nsfc-proposal --format json` 通过。
- 默认配置通过校验。
...
- `scholar-writing/scripts/validate.py`
- `scholar-writing/schemas/config.schema.yaml`
- `scholar-writing/schemas/scores.schema.yaml`
- `scholar-writing/schemas/manifest.schema.yaml`
- `scholar-writing/templates/nsfc/base.yaml`
- `scholar-writing/skills/writer/nsfc/*/config.yaml`
- `examples/**`
- `tests/**`
...
- `scholar-writing/scripts/run_pipeline.py`
- `scholar-writing/scripts/pipeline_state.py`
- `scholar-writing/scripts/check_sections.py`
- `docs/codex-architecture.md`
